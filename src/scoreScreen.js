...
Namespace('Crossword').ScoreScreen = (function() {
	...
	const _updateIsMobile = function() {
		...
		if (newMobile !== _isMobile) {
			...
			if (!newMobile) { // mobile -> desktop
				...
				setTimeout(()=>_updateClue(true), 400);
			}
		}
	};
	...
	const update = function(qset, scoreTable) {
		...
		forEveryQuestion((i, letters, x, y, dir, response) => forEveryLetter(x, y, dir, letters, response, function(left, top, l) {
			...
			if (!answersShown) { letterElement.innerHTML = response[l]; }
		}));
	};
	...
	var _drawBoard = function() {
		...
		forEveryQuestion(function(i, letters, x, y, dir, response, hinted) {
			...
			forEveryLetter(x, y, dir, letters, response, function(letterLeft, letterTop, l) {
	...
	var _highlightPuzzleWord = function(index) {
		...
		forEveryQuestion(function(i, letters, x, y, dir, response) {
			if (~~i === ~~index) {
				forEveryLetter(x,y,dir,letters,response, function(letterLeft, letterTop) {
	...
	var _toggleAnswers = function(e) {
		...
		if (e.target.checked) {
			...
			forEveryQuestion((i, letters, x, y, dir, response) => forEveryLetter(x, y, dir, letters, response, (left, top, l) => $(`#letter_${left}_${top}`)[0].innerHTML = letters[l]));
		} else {
			$('.letter').removeClass('neutral');
			forEveryQuestion((i, letters, x, y, dir, response) => forEveryLetter(x, y, dir, letters, response, (left, top, l) => $(`#letter_${left}_${top}`)[0].innerHTML = response[l]));
		}
	};
